...
def changing_name(name):
    ...
    if name == 'Golden':
        ...
        name = '        The outcomes of this injection are identical to the initial values of the registers.'
...
def comparing_physical(file):
    ...
    """
    fault = pd.read_csv(file)  # Reading the file containing the fault injection outcomes
...
def comparing_results_by_line(line, number_of_experiences, pdf):
    ...
    for i in range(models.shape[1]):  # Reading the fault models simulation outcomes file by column
        ...
        while boolean and j < models.shape[0]:
            boolean = (models.iloc[:, i][j] == line[j])  # The boolean is set to false if the cells are different
...
def compare_RTL(RTL_file):
    ...
    for RTL_outcome in range(1, RTL_outcomes.shape[1]):
        ...
        for i in range(models.shape[1]):  # Reading the fault models simulation outcomes file by column
            ...
            """
            while boolean and j < models.shape[0]:
                boolean = (models.iloc[:, i][j] == RTL_outcomes[RTL_outcome][
                    j])  # The boolean is set to false if the cells are different
                j += 1  # We increase our counter
            """ 
                If the variable boolean is true then we append to the list the name of this column which is the fault 
                model that occurred
        ...
        pdf.set_font("Arial", 'B', size=15)
        pdf.set_text_color(239, 124, 142)
        pdf.multi_cell(195, 10, 'The output of the RTL Simulation is :', 0, 1, '')

        pdf.set_font("Arial", size=12)
        pdf.set_text_color(0, 0, 0)
        pdf.multi_cell(195, 5, '{}'.format(RTL_outcomes[RTL_outcome].to_list()), 0, 1, '')
    ...
    pdf.output("report_RTL_injection.pdf")